...
| 节点间（K8s） | KubeOVN / CNI 网络加密策略（按部署 profile 启用）| ANI Installer + CNI 配置 |
...
```yaml
# kube-apiserver 原生配置：启用 Secret 加密
...
spec:
  runtimeClassName: gvisor    # 通过原生 Kubernetes RuntimeClass + containerd shim 接入
  containers:
    - name: agent
```
