...
- 契约入口层（ANI Gateway）：Go + Hertz，统一承载 Core OpenAPI REST API、Services OpenAPI REST API、SSE 和 SDK 调用
- ANI Core 内部实现层：Go 服务、controller、adapter，可通过 gRPC 内部通信；gRPC 不是 Services 绕过 Core OpenAPI 的跨层契约
- ANI Services 层：云服务、人机交互展现、AI 全生命周期、AI-Native 应用和功能编排，通过 Core OpenAPI REST API / Core SDK 组合基础设施能力
- 基础设施层：上游原生 Kubernetes + KubeOVN + HAMi + Volcano 等经 ports/adapters 封装的开源组件
...
2. **技术栈耦合风险**
   - Hertz + grpc-gateway 作为 Core 内部 REST/gRPC 转译组合，在实际工程中是否存在已知的兼容性问题？
   - Core 内部 Go 服务使用 gRPC、Services 跨层调用使用 OpenAPI REST/SDK，这种边界是否清晰？是否会导致双契约漂移、延迟或故障定位困难？
...
触发条件（已知部分）：
- 用户 POST /api/v1/svc/inference-services → 写 DB（status=pending）→ 创建 K8s CRD
...
操作 A：部署推理服务
步骤 1：POST /api/v1/svc/inference-services → ANI Gateway 接收
步骤 2：ANI Services 推理编排服务写入 Services 元数据（status=pending）
步骤 3：ANI Services 通过 Core SDK 调用 Core workload/object/encryption 等控制面 API
步骤 4：ANI Core 内部 service/controller 经 adapter 创建或更新 Kubernetes 资源
步骤 5：Init Container 运行（下载 + 解密模型）
步骤 6：vLLM 启动，健康检查通过
步骤 7：Core controller 观察 provider status → 回写 Core operation/resource status
步骤 8：ANI Services 轮询或订阅 Core 状态 → 更新 Services 业务状态为 running
步骤 9：发布 NATS 事件 → ANI Gateway SSE 推送前端

操作 B：上传文档到知识库
步骤 1：POST /api/v1/svc/knowledge-bases/{id}/documents（multipart 文件上传）
步骤 2：写 MinIO 存储原始文件
...
步骤 1：POST /api/v1/svc/models/import → 创建 DB 记录（status=pending）+ async_task
...
1. **API 层越权攻击路径**
   ...
   - 场景1：A 猜测到 B 的 knowledge_base UUID，直接调用 GET /api/v1/svc/knowledge-bases/{b_kb_id}
...
3. **幂等性与安全方法**
   - 所有 GET 接口是否真的是无副作用的？
   - POST /api/v1/svc/inference-services 触发异步操作，重复调用会创建多个服务还是幂等？
