...
> 最后更新：2026-05-23（修正 Kubernetes 底座为上游原生 Kubernetes）
...
|---|---|---|---|---|
| 裸物理服务器 | ANI Installer 全新安装上游原生 Kubernetes | KubeOVN | Rook-Ceph / 本地 SSD | 客户自购服务器，无任何基础设施 |
| 虚拟机（KVM/VMware/Hyper-V）| ANI Installer 全新安装上游原生 Kubernetes | KubeOVN | Rook-Ceph / vSAN | 客户有虚拟化平台，新建 VM 部署 |
| 云主机（阿里云/华为云/AWS ECS）| ANI Installer 全新安装上游原生 Kubernetes | KubeOVN / CNI-Hybrid | 云盘 CSI + MinIO | 客户在公有云租用 ECS |
...
- ANI 底座使用上游原生 Kubernetes，不绑定特定 Kubernetes 发行版；版本、API、RuntimeClass、CSI、CNI、CRD 语义必须与开源社区同步。
...
Step 4   部署模式      全新原生 K8s | 复用已有 K8s（attach-k8s 流程）
...
| 模块 | 语言 | 工期 | 纳入月份 |
|---|---|---|---|
| `ani-installer` 主体（TUI + 向导 + 原生 Kubernetes bootstrap）| Go | 3 周 | M1 |
...
- ✅ 内部 CA + 纯 IP HTTPS 访问
- ✅ 计量基础版（用量记录，无账单）
- ⏳ vCluster 租户 K8s 集群能力由 ANI Core Sprint 5 建立 local-profile 与真实环境门禁；Installer 负责部署所需底座，真实 vCluster provider 跑通后才能标记为交付完成
