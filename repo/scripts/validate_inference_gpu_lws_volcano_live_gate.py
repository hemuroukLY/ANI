#!/usr/bin/env python3
"""Validate the GPU/LWS/Volcano InferenceService live gate contract."""
...
DEFAULT_GATE = ROOT / "deploy/real-k8s-lab/inference-gpu-lws-volcano-live-gate.yaml"
DEFAULT_EVIDENCE = ROOT / "development-records/live-evidence/inference-gpu-lws-volcano-live-20260817.json"
PROFILE = "INFERENCE-SERVICE-GPU-LWS-VOLCANO-LIVE-GATE-C20"
REQUIRED_CHECKS = {
    "platform-workload-capabilities-probe",
    "cpu-deployment-keeps-default-scheduler",
    "gpu-single-node-uses-volcano",
    "gpu-create-fail-closed-without-plugin",
    "multi-node-lws-fail-closed-without-crd",
    "lws-renders-podgroup-and-leader-service",
    "gpu-lws-runtime-skipped-until-ready-cluster",
    "smoke-workload-untouched",
}


def fail(message: str) -> None:
    raise SystemExit(f"inference gpu lws volcano live gate invalid: {message}")
...
def main() -> None:
    ...
    print("inference gpu lws volcano live gate valid")
