#!/usr/bin/env python3
"""Validate the CPU vLLM InferenceService ops live gate contract."""
...
DEFAULT_GATE = ROOT / "deploy/real-k8s-lab/inference-cpu-vllm-ops-live-gate.yaml"
DEFAULT_EVIDENCE = ROOT / "development-records/live-evidence/inference-cpu-vllm-ops-live-20260815.json"
PROFILE = "INFERENCE-SERVICE-CPU-VLLM-OPS-LIVE-GATE-C15"
REQUIRED_CHECKS = {
    "inference-service-product-logs",
    "inference-service-scale-desired-replicas",
    "inference-service-scale-back-running",
    "inference-service-lab-restart-get",
    "smoke-workload-untouched",
}


def fail(message: str) -> None:
    raise SystemExit(f"inference cpu vllm ops live gate invalid: {message}")
...
def validate_evidence(path: Path) -> None:
    ...
    if evidence.get("engine") != "vllm-cpu":
        fail("evidence must record vllm-cpu")
...
def main() -> None:
    ...
    print("inference cpu vllm ops live gate valid")
