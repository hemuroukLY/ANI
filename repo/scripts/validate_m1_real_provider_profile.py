#!/usr/bin/env python3
"""Offline checks for the M1 real provider regression profile."""
...
REQUIRED_KEYWORDS = (
    ...
    "default local remains safe",
    "execution switches required",
)
...
def main() -> int:
    root = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else pathlib.Path("deploy/manifests/m1-e2e-b")
    text = load_text(root)
    ...
    print(f"validated M1 real provider profile under {root}")
    return 0
