...
import validate_inference_cpu_vllm_live_gate as gate


class InferenceCPUVllmLiveGateTests(unittest.TestCase):
    ...
    def test_rejects_live_status_without_evidence(self) -> None:
        ...
        document["evidence"] = "development-records/live-evidence/missing-inference-cpu-vllm.json"
    ...
    def test_accepts_live_status_with_redacted_evidence(self) -> None:
        ...
        target = gate.DEFAULT_EVIDENCE.with_name("inference-cpu-vllm-live-test.json")
    ...
    def test_rejects_missing_health_smoke_check(self) -> None:
        document = gate.load_gate(gate.DEFAULT_GATE)
        document["live_checks"] = [
            item for item in document["live_checks"] if item.get("id") != "inference-service-running-health-smoke"
        ]
        with self.assertRaises(SystemExit):
            gate.validate_contract(document)
