#!/usr/bin/env python3
"""Validate the Console-shaped CPU InferenceService live gate contract."""
...
DEFAULT_GATE = ROOT / "deploy/real-k8s-lab/inference-console-shaped-e2e-live-gate.yaml"
DEFAULT_EVIDENCE = ROOT / "development-records/live-evidence/inference-console-shaped-e2e-live-20260817.json"
...
def fail(message: str) -> None:
    raise SystemExit(f"inference console-shaped e2e live gate invalid: {message}")
...
def validate_contract(document: dict[str, Any]) -> None:
    ...
    required_endpoints = {"ani_console_nginx_api_proxy", "ani_gateway_inference_services_api", "inference_control_grpc"}
    if not isinstance(endpoints, list) or required_endpoints - set(endpoints):
        fail("required_endpoints must include Console nginx, Gateway inference HTTP, and inference gRPC")
...
def main() -> None:
    ...
    print("inference console-shaped e2e live gate valid")
