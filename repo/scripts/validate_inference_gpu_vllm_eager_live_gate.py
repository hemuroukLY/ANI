#!/usr/bin/env python3
"""Validate the GPU vLLM enforce-eager live gate."""
...
DEFAULT_GATE = ROOT / "deploy/real-k8s-lab/inference-gpu-vllm-eager-live-gate.yaml"
DEFAULT_EVIDENCE = ROOT / "development-records/live-evidence/inference-gpu-vllm-eager-live-20260818.json"
PROFILE = "INFERENCE-SERVICE-GPU-VLLM-EAGER-LIVE-GATE-C34"
GATEWAY_EVIDENCE = "ani-system/ani-gateway"
REQUIRED_CHECKS = {
    "gpu-vllm-launch-enforce-eager",
    "live-pod-ready",
    "runtime-health-200",
    "product-get-running",
    "invocation-url-null",
...
def fail(message: str) -> None:
    raise SystemExit(f"inference gpu vllm eager live gate invalid: {message}")
...
def validate_evidence(path: Path) -> None:
    ...
    if evidence.get("launch") != "enforce-eager":
        fail("evidence must record GPU vLLM enforce-eager launch")
    if evidence.get("pod_ready") is not True:
        fail("evidence must record the retained GPU pod as Ready")
    if evidence.get("health") != 200:
        fail("evidence must record runtime /health 200")
...
def main() -> None:
    ...
    print("inference gpu vllm eager live gate valid")
