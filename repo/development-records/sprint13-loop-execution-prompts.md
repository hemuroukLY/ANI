...
| # | 切片 | 真实组件（已选） | status |
|---|---|---|---|
| S01 | 网络路由 | Kube-OVN v1.15.8 | real-provider evidence passed, production-shaped acceptance standard ready / evidence pending（route metadata persistence + in-cluster RBAC profile complete；见 `sprint13-b-track-production-shaped-closure.md`；historical evidence：`live-evidence/sprint13-netroute-kubeovn-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`） |
| S02 | K8s workloads | vCluster / K8s v1.36.1 | real-provider evidence passed, production-shaped acceptance standard ready / evidence pending（`validate-vcluster-live-gate --production-shaped` 固定 metadata target + TLS/token 标准；historical evidence：`live-evidence/sprint13-k8s-workloads-vcluster-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`） |
| S03 | 卷快照 / mount-targets | Rook-Ceph（CSI snapshot / RBD / NFS） | real-provider evidence passed, production-shaped acceptance standard ready / evidence pending（`validate-storage-live-gate --production-shaped` 固定 production gateway + in-cluster RBAC 标准；historical evidence：`live-evidence/sprint13-storage-rook-ceph-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`） |
| S04 | GPU 清单 / 占用 | NVIDIA device-plugin / DCGM | real-provider evidence passed, production-shaped acceptance standard ready / evidence pending（`validate-gpu-inventory-live-gate --production-shaped` 固定 in-cluster Kubernetes API + cluster Service/Prometheus 标准；historical evidence：`live-evidence/sprint13-gpu-inventory-dcgm-live-evidence.json`；readiness：`sprint13-gpu-inventory-dcgm-readiness.md`；production guard：`validate-sprint13-b-track-production-shape`） |
...
排序原则：S01–S04 已补齐 production-shaped acceptance standard，但 historical lab evidence 仍不能直接升级为 production passed；S05–S07 的真实后端需 B 轨人工部署，且必须复用 S01-S04 已建立的 production-shaped proof_items 标准。
