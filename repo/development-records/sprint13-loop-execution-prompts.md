...
| # | 切片 | 真实组件（已选） | status |
|---|---|---|---|
| S01 | 网络路由 | Kube-OVN v1.15.8 | real-provider evidence passed, production-shaped pending（见 `sprint13-netroute-kubeovn-live-result.md`；evidence：`live-evidence/sprint13-netroute-kubeovn-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`） |
| S02 | K8s workloads | vCluster / K8s v1.36.1 | real-provider evidence passed, production-shaped pending（见 `sprint13-k8s-workloads-vcluster-live-result.md`；evidence：`live-evidence/sprint13-k8s-workloads-vcluster-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`） |
| S03 | 卷快照 / mount-targets | Rook-Ceph（CSI snapshot / RBD / NFS） | real-provider evidence passed, production-shaped pending（见 `sprint13-storage-rook-ceph-live-result.md`；evidence：`live-evidence/sprint13-storage-rook-ceph-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`） |
| S04 | GPU 清单 / 占用 | NVIDIA device-plugin / DCGM | real-provider evidence passed, production-shaped pending（见 `sprint13-gpu-inventory-dcgm-live-result.md`；evidence：`live-evidence/sprint13-gpu-inventory-dcgm-live-evidence.json`；readiness：`sprint13-gpu-inventory-dcgm-readiness.md`；production guard：`validate-sprint13-b-track-production-shape`） |
