...
> Sprint 14 分支执行：`feature/sprint14-core-resilience-semantics` 已完成 R-P0-0 gateway shared store 前置批次、R-P0-1 gateway rate limit、R-P0-2 gateway idempotency replay、R-P0-3 adapter per-call timeout 与 R-P0-4 data-plane readyz health；这些批次仅为 local/logic verified，不声明 production ready。R-P0-4 的 `validate-readyz-dataplane-live-gate` 当前只执行 local gate，未执行真实后端 kill。
