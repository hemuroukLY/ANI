...
当前重心：Sprint 13 / Core real provider 与 live gate 收敛（2026-06-19 起）；Sprint 12 已完成 A/B1/B2/B3 全部 19 个已声明未实现 Core handler + 2 个 422 的 Tier1 local profile，并完成文档闭环；Sprint 13 必须从这些既有代码边界接入真实 provider 与固定 live gate：B1 边界为 `pkg/ports/instance_observability.go`、`pkg/ports/gpu_inventory.go`、`pkg/ports/sandbox_template_catalog.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler；B2 边界为 `pkg/ports/network_resources.go`、`pkg/ports/storage_resources.go`、`pkg/ports/k8s_clusters.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler，S01 网络路由 Kube-OVN 已完成 Core route provider 与 Gateway runtime 接线并通过真实 route live gate；B3 边界为 `pkg/ports/storage_resources.go`、`pkg/ports/object_store.go`、`pkg/ports/vector_store.go`、`pkg/adapters/runtime/storage_service.go`、`pkg/adapters/runtime/vector_store_service.go` 与 Gateway handler；未执行对应 live gate 前，不代表 real-provider、runtime ready 或 production ready；详见 repo/CURRENT-SPRINT.md
当前执行入口：Sprint 13 真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN 已通过 B 轨真实 live gate（evidence：repo/development-records/live-evidence/sprint13-netroute-kubeovn-live-evidence.json；result：repo/development-records/sprint13-netroute-kubeovn-live-result.md），S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM、S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已完成 A 轨 code+contract ready（LIVE PENDING），B 轨执行时必须先声明真实组件与版本、live gate 命令、evidence 输出路径和失败边界
...
| Sprint 13 ⭐ | 进行中 | 2026-06-19 起 | 真实 provider / live gate 收敛；从 Sprint 12 已建立的 ports/adapters/router 边界接入 K8s/kubelet/Prometheus/DCGM/Kata/Kube-OVN/Rook-Ceph/MinIO/向量后端等真实组件；S01 网络路由 Kube-OVN 已通过真实 route live gate 并归档 evidence；S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM、S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已到 code+contract ready / LIVE PENDING（`SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK` / `validate-instance-observability-live-gate` 固定 Prometheus + kubelet / K8s API logs/events/metrics/security-events/exec contract）；计划见 `repo/development-records/sprint13-real-provider-readiness-plan.md`，未执行对应 live gate 前不得标 runtime/production ready |
