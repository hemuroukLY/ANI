...
> 最后更新：2026-06-20（Sprint 12 / Core「Services 支撑 Handler」已完成 A/B1/B2/B3 全部 19 个 Core handler + 2 个 422 的 Tier1 local profile 收口；当前进入 Sprint 13 / Core real provider 与 live gate 收敛；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph 与 S04 GPU NVIDIA device-plugin/DCGM 已通过真实 live gate 并归档 historical lab evidence；`SPRINT13-B-TRACK-PRODUCTION-SHAPED-CLOSURE` 与 post-review hardening 已补齐 in-cluster ServiceAccount/RBAC、Gateway + bootstrap Kubernetes REST provider 装配、S01 route metadata persistence、S02/S03/S04 production-shaped live gate flags 与 S01-S07 proof_items 标准，但当前 S01-S04 旧 evidence 仍为 `production_shape.status=pending`，必须重新跑 production-shaped live gate 后才可标 passed；S05-S07 已完成 A 轨 code+contract ready / LIVE PENDING；本仓库只推进 ANI Core，不开发 Services/RAG/Console/BOSS/frontends；Sprint 11 / Core Real Deployment Validation 与 Rook-Ceph 正式部署结果继续作为历史回归边界保留；Sprint 13 未跑通对应 production-shaped live gate 前，不得把对应能力标记为 runtime ready 或 production ready）
...
当前重心：Sprint 13 / Core real provider 与 live gate 收敛（2026-06-19 起）；Sprint 12 已完成 A/B1/B2/B3 全部 19 个已声明未实现 Core handler + 2 个 422 的 Tier1 local profile，并完成文档闭环；Sprint 13 必须从这些既有代码边界接入真实 provider 与固定 live gate：B1 边界为 `pkg/ports/instance_observability.go`、`pkg/ports/gpu_inventory.go`、`pkg/ports/sandbox_template_catalog.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler；S04 GPU 已补齐 `GPU_INVENTORY_PROVIDER=kubernetes_rest` 接线路径，并通过 DCGM exporter + Core GPU inventory live gate；B2 边界为 `pkg/ports/network_resources.go`、`pkg/ports/storage_resources.go`、`pkg/ports/k8s_clusters.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler，S01 网络路由 Kube-OVN 已完成 Core route provider 与 Gateway/bootstrap runtime 接线并通过真实 route live gate，S02 K8s workloads vCluster 已通过真实 workload list live gate，S03 storage Rook-Ceph 已通过真实 snapshot/mount-target live gate；S01-S04 现已完成 production-shaped acceptance standard closure（in-cluster ServiceAccount/RBAC、Gateway + bootstrap Kubernetes REST provider 装配、route metadata persistence、production-shaped live gate flags 与 proof_items 标准），但旧 evidence 仍受 `validate-sprint13-b-track-production-shape` 约束为 pending，未重新跑 production-shaped live gate 前不得标 runtime/production ready；B3 边界为 `pkg/ports/storage_resources.go`、`pkg/ports/object_store.go`、`pkg/ports/vector_store.go`、`pkg/adapters/runtime/storage_service.go`、`pkg/adapters/runtime/vector_store_service.go` 与 Gateway handler；未执行对应 live gate 前，不代表 real-provider、runtime ready 或 production ready；详见 repo/CURRENT-SPRINT.md
...
最近里程碑（详情见 repo/CURRENT-SPRINT.md 已完成切片）：
  2026-06-20  SPRINT13-B-TRACK-PRODUCTION-SHAPED-POST-REVIEW：S01-S04 B 轨 production-shaped 深度复审完成；bootstrap `Config` 与 `NewCapabilitiesWithConfig` 统一支持 in-cluster ServiceAccount Kubernetes REST provider 装配，覆盖 S01 network、S03 storage、S04 GPU inventory 与 S07 Prometheus observability；adapter 层不再隐式读取 ambient env；旧 S01-S04 lab evidence 仍为 `production_shape.status=pending`，必须 production-shaped rerun 后才可标 passed [Production-shaped hardening]
  2026-06-20  SPRINT13-B-TRACK-PRODUCTION-SHAPED-CLOSURE：S01-S04 B 轨 production-shaped closure 完成；Kubernetes REST client 支持 in-cluster ServiceAccount token/CA，Gateway network/storage/gpu runtime 透传 in-cluster 配置，S01 route metadata 新增持久化与 migration，S02/S03/S04 live gate 增加 `--production-shaped` 模式，production profile/RBAC 固定 S01-S07 proof_items 标准；旧 S01-S04 lab evidence 仍为 `production_shape.status=pending`，必须 production-shaped rerun 后才可标 passed [Production-shaped closure]
  2026-06-20  SPRINT13-B-TRACK-PRODUCTION-SHAPE-REVIEW：S01-S04 B 轨生产形态边界审查完成；新增 `validate-sprint13-b-track-production-shape`，强制四份 evidence JSON 写入 `production_shape.status=pending` 与生产形态缺口，禁止把 lab kubeconfig、kubectl proxy、port-forward 或 dev gateway evidence 误标为 production ready [Guard]
...
| Sprint 13 ⭐ | 进行中 | 2026-06-19 起 | 真实 provider / live gate 收敛；从 Sprint 12 已建立的 ports/adapters/router 边界接入 K8s/kubelet/Prometheus/DCGM/Kata/Kube-OVN/Rook-Ceph/MinIO/向量后端等真实组件；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph 与 S04 GPU NVIDIA device-plugin/DCGM 已通过真实 live gate 并归档 historical lab evidence；production-shaped acceptance standard 已补齐 Gateway/bootstrap 代码路径、部署契约和门禁，但 S01-S04 旧 evidence 仍 pending，必须重新跑 production-shaped live gate 后才可标 passed；S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已到 code+contract ready / LIVE PENDING（`SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK` / `validate-instance-observability-live-gate` 固定 Prometheus + kubelet / K8s API logs/events/metrics/security-events/exec contract）；计划见 `repo/development-records/sprint13-real-provider-readiness-plan.md`，未执行对应 production-shaped live gate 前不得标 runtime/production ready |
