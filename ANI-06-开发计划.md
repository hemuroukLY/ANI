...
> 最后更新：2026-06-20（Sprint 12 / Core「Services 支撑 Handler」已完成 A/B1/B2/B3 全部 19 个 Core handler + 2 个 422 的 Tier1 local profile 收口；当前进入 Sprint 13 / Core real provider 与 live gate 收敛；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM 与 S05 object-store MinIO pre-signed URL 已通过 production-shaped live gate 并归档 `production_shape.status=passed` evidence；S01 强制经 ANI Gateway create/list 后再观测 Kube-OVN 底层对象，S02-S04 分别固定 metadata target TLS、in-cluster RBAC、cluster Service metrics proof_items 与 workload/storage/GPU+DCGM 正向业务证据，S05 固定 MinIO bucket create/list、upload/download pre-signed URL、实际 PUT/GET 与 cleanup evidence；SPRINT13-AUTH-DEX-PRODUCTION-GATE（Auth/Dex production gate）已通过真实集群生产形态验证，production-shaped Gateway 使用 `ANI_AUTH_MODE=auth_service`，并经 `validate-auth-dex-production-gate` 固定 anonymous 401、OIDC begin/complete 200、protected API bearer 200 与 refresh 200；S01-S05 的 Auth/Dex production ready 阻断已解除，但该结论仍不是 full platform production ready；S06-S07 B 轨可以继续，且已完成 A 轨 code+contract ready / LIVE PENDING；本仓库只推进 ANI Core，不开发 Services/RAG/Console/BOSS/frontends；Sprint 11 / Core Real Deployment Validation 与 Rook-Ceph 正式部署结果继续作为历史回归边界保留；S01-S05 production-shaped acceptance passed 不等于 full platform production ready，Sprint 13 未跑通对应 production-shaped live gate 前不得把 S06-S07 标记为 runtime ready 或 production ready）
...
| Sprint 13 ⭐ | 进行中 | 2026-06-19 起 | 真实 provider / live gate 收敛；从 Sprint 12 已建立的 ports/adapters/router 边界接入 K8s/kubelet/Prometheus/DCGM/Kata/Kube-OVN/Rook-Ceph/MinIO/向量后端等真实组件；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM 与 S05 object-store MinIO 已通过 production-shaped live gate 并归档 `production_shape.status=passed` evidence；S06 vector Milvus 与 S07 instance observability Prometheus 已到 code+contract ready / LIVE PENDING（`SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK` / `validate-instance-observability-live-gate` 固定 Prometheus + kubelet / K8s API logs/events/metrics/security-events/exec contract）；计划见 `repo/development-records/sprint13-real-provider-readiness-plan.md`，未执行对应 production-shaped live gate 前不得标 runtime/production ready |
