...
> 本文定义开源组件适配边界和 ports/adapters 强制规则，不作为当前开发进度来源。当前阶段以 `ANI-DOCS-INDEX.md`、`ANI-06-开发计划.md` Section 零和 `repo/CURRENT-SPRINT.md` 为准。
...
ANI 选择开源组件不是“谁新选谁”，也不是只看单一性能指标。组件必须先通过准入评估，再进入默认技术栈：

| 准入维度 | 必须检查 | 目的 |
|---|---|---|
| 社区热度 | GitHub stars、forks、contributors、issue/PR 活跃度、release 频率 | 选择被充分使用和验证的项目，降低踩坑概率 |
| 稳定成熟 | 生产案例、版本兼容策略、长期维护者、CVE/安全响应、升级记录 | 避免把新项目、实验项目或维护不确定项目放入核心路径 |
| 性能与效率 | 基准测试、资源占用、横向扩展方式、故障恢复能力 | 支撑 ANI 生产级云服务，而不是只在 demo 中可用 |
| 源码与文档 | 源码可读、架构清晰、API 文档完整、运维文档丰富、配置项可解释 | 让人和 AI 都能读懂、排障、修 bug、生成运维脚本 |
| 运维运营 | metrics、logs、health check、backup/restore、upgrade/rollback、离线部署 | 方便 ANI 最终运维和运营，而不是只方便开发 |
| 可替换性 | License、协议兼容、数据迁移方式、替代组件、adapter 边界 | 默认组件可插拔替换，避免产品被单个组件锁死 |

硬规则：

- 不得仅因 GitHub stars 高就选型；stars 只是准入信号，不是最终决策。
- 核心路径默认选择成熟、稳定、文档丰富、源码可审计的项目；除非 CTO/架构负责人批准，不把新开源项目放入 P0 主链路。
- 每个默认组件必须说明“为什么选它、如何运维、如何替换、替换影响哪些文件、失败时如何降级或回滚”。
- 如果组件源码和文档不足以让 AI 开发工具理解、定位 bug、生成测试或生成运维动作，不得进入 P0 默认组件清单。
...
### 2.3 Kubernetes 是唯一允许稳定绑定的底层平台 API
...
- 是否为新增默认组件补齐准入评估：GitHub stars/forks/contributors、release 频率、issue/PR 活跃度、License、生产案例、CVE 响应、文档和源码可读性。
- 是否说明该组件如何运维运营：metrics/logs/health check、backup/restore、upgrade/rollback、离线部署和常见故障处理。
...
- 是否记录替换该组件时影响哪些文件。
- 是否记录替代组件、数据迁移路径和回滚方式，确保默认组件可插拔替换。

出现以下情况必须阻止合并：

- 只因为新项目热度、短期性能或宣传材料就把它放入 P0 主链路，且没有稳定性、文档、运维和替换评估。
