...
Sprint 13 计划：真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM 与 S05 object-store MinIO pre-signed URL 已通过 `--production-shaped` live gate，五份 evidence 均为 `production_shape.status=passed`，并由 `validate-sprint13-b-track-production-shape` 固定 Gateway / in-cluster ServiceAccount / metadata target / cluster Service / object-store proof_items 标准以及 S02/S03/S04/S05 workload/storage/GPU+DCGM/object-store 正向业务证据；S01 生产形态门禁强制经 ANI Gateway `POST/GET /networks/routes` create/list，再用 kubectl 只做底层 Kube-OVN 观测与 cleanup，禁止 kubectl-only 证据标 passed；SPRINT13-GPU-INVENTORY-DCGM-A-TRACK / validate-gpu-inventory-live-gate 固定 NVIDIA device-plugin node GPU capacity 与 DCGM metrics 的 GPU inventory/occupancy live gate 契约门禁；SPRINT13-OBJECTSTORE-MINIO-A-TRACK / validate-object-store-live-gate 固定 MinIO bucket 与 upload/download pre-signed URL contract 门禁，SPRINT13-OBJECTSTORE-MINIO-LIVE-A 已归档真实 bucket create/list、pre-signed URL PUT/GET 与 cleanup evidence；SPRINT13-VECTOR-MILVUS-A-TRACK / validate-vector-store-live-gate 固定 Milvus vector document insert 与 search readiness contract 门禁；SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK / validate-instance-observability-live-gate 固定 Prometheus + kubelet / K8s API instance observability logs/events/metrics/security-events/exec contract 门禁；S01-S05 只代表 production-shaped acceptance passed，不等于 full platform production ready；SPRINT13-AUTH-DEX-PRODUCTION-GATE（Auth/Dex production gate）已通过真实集群生产形态验证，production-shaped Gateway 使用 `ANI_AUTH_MODE=auth_service`，并经 `validate-auth-dex-production-gate` 固定 anonymous 401、OIDC begin/complete 200、protected API bearer 200 与 refresh 200；S01-S05 的 Auth/Dex production ready 阻断已解除，但该结论仍不是 full platform production ready；S06-S07 B 轨可以继续，但 S06-S07 未跑通 production-shaped live gate 前不得标 runtime/production ready，仍为 LIVE PENDING

兼容硬门禁 token：`S05-S07 B 轨可以继续` 仍表示 Auth/Dex production gate 后允许按组件 production-shaped 标准推进 S05-S07；截至 2026-06-20，S05 已 passed，剩余待执行切片为 S06-S07。
