...
Sprint 13 计划：真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN 已通过 `validate_kubeovn_network_live_gate.py --live --cleanup` 并归档 historical lab evidence；S02 K8s workloads vCluster 已通过 `validate_vcluster_live_gate.py --live --proxy-server ... --chart-version 0.34.1` 并归档 workload list historical lab evidence；S03 storage Rook-Ceph 已通过 `validate_storage_live_gate.py --live` 并归档 snapshot/mount-target historical lab evidence，A 轨 contract 记录为 SPRINT13-STORAGE-ROOK-CEPH-A-TRACK / validate-storage-live-gate；S04 GPU 已通过 `validate_gpu_inventory_live_gate.py --live` 并归档 GPU inventory/occupancy + DCGM historical lab evidence，SPRINT13-GPU-INVENTORY-DCGM-A-TRACK / validate-gpu-inventory-live-gate 固定 NVIDIA device-plugin / DCGM GPU inventory 与 occupancy contract 门禁，当前可标 real-provider evidence passed for S04 GPU inventory gate；SPRINT13-B-TRACK-PRODUCTION-SHAPED-CLOSURE 与 post-review hardening 已补齐 in-cluster ServiceAccount/RBAC、Gateway + bootstrap Kubernetes REST provider 装配、S01 route metadata persistence、S02/S03/S04 `--production-shaped` live mode 与 S01-S07 proof_items 标准；S01-S04 旧 evidence 仍必须显式记录 `production_shape.status=pending`，不得把 lab kubeconfig、kubectl proxy、port-forward 或 dev gateway 证据标为 production ready，只有重新跑 production-shaped live gate 后才可标 passed；SPRINT13-OBJECTSTORE-MINIO-A-TRACK / validate-object-store-live-gate 固定 MinIO bucket 与 upload/download pre-signed URL contract 门禁；SPRINT13-VECTOR-MILVUS-A-TRACK / validate-vector-store-live-gate 固定 Milvus vector document insert 与 search readiness contract 门禁；SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK / validate-instance-observability-live-gate 固定 Prometheus + kubelet / K8s API instance observability logs/events/metrics/security-events/exec contract 门禁；S05-S07 未跑通 production-shaped live gate 前不得标 runtime/production ready，仍为 LIVE PENDING
...
Sprint 6-10 完成 contract/local/release-prep scaffold（installer、offline、CLI、RC readiness 均为 contract/local validation，非真实发布）；Sprint 11 为首次真实物理服务器验证阶段，包含 Rook-Ceph VM 优先块存储 live 部署，现已转为历史回归门禁；Sprint 12 已完成 Core「Services 支撑 Handler」19 个 handler + 2 个 422 的 Tier1 local profile 收口。历史 Sprint 4 回归门禁（`SPEC-SPLIT-A`、`SPEC-CORE-BETA`、`SPEC-COMPAT-A`、`MOCK-A`、`DOC-API-A`、`SDK-BETA-*`、`SDK-MOCK-SMOKE-*`、`SPRINT4-CLOSURE-A`）有效。详细技术边界与验收命令见 [`repo/CURRENT-SPRINT.md`](repo/CURRENT-SPRINT.md)，已完成批次见 [`repo/development-records/README.md`](repo/development-records/README.md)。当前活跃冲刺为 **Sprint 13 / Core real provider 与 live gate 收敛**：S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph 与 S04 GPU NVIDIA device-plugin/DCGM 已通过真实 live gate 并归档 historical lab evidence；production-shaped acceptance standard 已由 `SPRINT13-B-TRACK-PRODUCTION-SHAPED-CLOSURE` 与 post-review hardening 补齐 Gateway/bootstrap 代码路径、部署契约和门禁，但 S01-S04 旧 evidence 仍为 pending，必须重新跑 production-shaped live gate 才可标 passed；S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已完成 A 轨 code+contract ready / LIVE PENDING；从 Sprint 12 已闭合的 handler、ports、local adapters 接入真实组件和 evidence JSON 门禁；未跑通对应 production-shaped gate 前，对应能力不得标 runtime/production ready。
