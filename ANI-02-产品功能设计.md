...
5. Services API 的真实来源是 `repo/api/openapi/services/v1.yaml`；当前只覆盖首批 `models`、`inference-services`、`knowledge-bases` 切片，完整前端功能与接口定义需在 2026-06-15 至 2026-06-20 输出后统一重置旧 Services 逻辑。
...
> 本节定义 ANI Services 的产品目标范围，不等于当前 Repo 中早期 Services 代码的最终边界。所有服务均由 Services 团队开发，通过 ANI Core OpenAPI REST API / Core SDK 实现，禁止直接操作底层基础设施或直接调用 Core 内部 gRPC service。2026-06-15 至 2026-06-20，Services 团队需输出完整前端功能、Services 功能和接口定义；评审通过后，Repo 中陈旧的 Services 逻辑按新定义删除或覆盖。
